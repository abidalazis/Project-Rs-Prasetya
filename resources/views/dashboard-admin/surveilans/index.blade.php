...
<div class="container-fluid px-4">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <a href="{{ route('tambah-surveilans.create') }}" class="btn btn-primary">Tambah</a>
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td>{{ $item->link }}</td>
                            <td>
                                <a href="/tambah-surveilans/ {{ $item->id }}/edit" class="btn btn-warning btn-sm">Edit</a>
                                <form onsubmit="return confirm('YAKIN AKAN MENGHAPUS?')" class="d-inline"
                                    action="/tambah-surveilans/ {{ $item->id }}" method="post">
                                    @csrf
                                    @method('DELETE')
